...
Par exemple :
    ...
    array(
        'css' => array(
            ...
            // concaténation simple - 
            // la parsing se fait à la volée lors de la requête
...
*   L'index de premier niveau correspond au type de fichier produit, parmi :
    *   `css`
    ...
    Tous les fichiers produits seront placés dans le répertoire `css` à la racine du repertoire d'installation.
    
*   L'index de second niveau définit le fichier à produire, au moyen de son
    chemin relatif à la racine pour ce type de fichier (par exemple, pour `mypath/myfile.css`,
...
*   Le tableau correspondant à cet index définit comment sera générée la cible.
    ...
    `deploy_parser`
    :   La configuration du parser à utiliser pour le déploiement (voir le § 
        correspondant).
...
:   un parser faisant la concaténation de css, puis les parsant au moyen
    ...
    Les parsers étant chargés au moyen de l'autoloader, il est 
    parfaitement possible de définir de nouvelles  classes de parser, 
    tant qu'elles implémentent les interfaces correspondantes.

`\Dcp\Style\dcpCssCopyDirectory`
:   un parser permettant de copier le contenu d'un répertoire. Ceci est utilisé notamment pour copier un ensemble d'images liés à la css.

## Créer un nouveau style à partir du style par défaut
...
#### Règles interfaces initiales
Les interfaces standard de dynacase utilise les css suivantes :

'dcp/main.css'
:   Cette css est utilisée dans la plupart des interfaces documentaire de haut niveau fourni par la module _dynacase-core_.
    Elle est utilisée pour les interface web de documents (consultation et modification) et pour les liste de documents.
...
`dcp/core.css`
:   Cette css est utilisé par défaut sur les interfaces si le template utilisé possède la clef `|CSS:REF]`. Elle est utilisé sur les interfaces de core d'importation et de gestion des droits.
...
:   Cette css est sur les vues standards de consultation des documents. La vue de consultation utilise aussi la css `dcp/main.css`.
...
`dcp/system.css`
:   Cette css est utilisé par défaut sur les interfaces si le template utilisé possède la clef `|CSS:REF]`. Elle est utilisé sur les ancinnes interfaces de core.
...
#### Règles interfaces jquery-ui

Le module `dynacase-jqueryui-installer` ajoute aussi deux autres règles au style `DEFAULT`.

 `dcp/jquery-ui.css`
 :  Cette css est utilisée dans les interfaces utilisant jquery-ui sauf dans l'interface GENERIC_LIST (liste de documents de ONEFAM) qui elle utilise une autre version de jquery-ui.
    Elle est composée des éléments suivants :
...
Si le parser employé est `dcpCssTemplateParser` alors les variables définis dans
 `$sty_const` et `$sty_local` seront utilisables.
...
<table><tr><td style='background-color:#4D4D4D'>COLOR_A0: #4D4D4D</td>
...
</tr><tr><td style='background-color:#126AB5'>COLOR_B0: #126AB5</td>
...
</tr><tr><td style='background-color:#CBC11F'>COLOR_C0: #CBC11F</td>
...
#### Fichier `css` avec flag RULE_FLAG_PARSE_ON_RUNTIME

Si la règle utilise le flag `Style::RULE_FLAG_PARSE_ON_RUNTIME` alors la css peut 
aussi utiliser les variables suivantes :
...
### Surcharge de `DEFAULT`


#### Définir son fichier de style avec héritage 
...
## Surcharger les images

De même, les images peuvent être surchargées. Pour cela il faut publier des images 
de même nom dans le répertoire `Images` du style.

Par exemple la famille _Groupe d'utilisateurs_ utilise l'image `igroup.png` comme 
icone. Si le style `MONSTYLE` publie l'image `STYLE/MONSTYLE/Images/igroup.png` 
...
Cette commande permet de réaliser les règles définies dans le style. Les
fichiers css sont alors produits dans le répertoire `css` à la racine du
répertoire d'installation.

L'option `--verbose=yes` permet de voir quelles sont les règles qui sont 
prises en compte.
